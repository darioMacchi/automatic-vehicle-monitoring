...
def read_local_test(env: StreamExecutionEnvironment):
    now_ms = int(time.time() * 1000)
    ts_old = (now_ms-2000)/1000.0
    ts_new = (now_ms-1000)/1000.0
    ...
    ds = env.from_collection(new_coll, type_info=Types.STRING())

    watermark_strategy = WatermarkStrategy.for_bounded_out_of_orderness(Duration.of_seconds(2)) \
                                        .with_timestamp_assigner(MyTimestampAssigner())
    
    ds = ds.assign_timestamps_and_watermarks(watermark_strategy).map(lambda s: json.loads(s))

    # window 10s slide 1s per test
    ds.key_by(lambda d: d['license_plate'], key_type=Types.STRING()) \
        .window(SlidingEventTimeWindows.of(Time.seconds(10), Time.seconds(1))) \
        .process(MyProcessWindowFunction()) \
        .print()
